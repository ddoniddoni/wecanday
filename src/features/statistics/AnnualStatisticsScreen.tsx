...
function YearNavigation({ canGoForward, onNext, onPrevious, t, theme, year }: { canGoForward: boolean; onNext: () => void; onPrevious: () => void; t: TFunction<'statistics'>; theme: AppTheme; year: string }) {
  return <View style={[styles.navigation, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
    <Pressable accessibilityLabel={t('annual.previousYear')} accessibilityRole="button" onPress={onPrevious} style={styles.navigationButton}><Text style={[styles.navigationButtonLabel, { color: theme.colors.primary }]}>‹</Text></Pressable>
    <Text accessibilityRole="header" style={[styles.yearLabel, { color: theme.colors.text }]}>{year}</Text>
    <Pressable accessibilityLabel={t('annual.nextYear')} accessibilityRole="button" accessibilityState={{ disabled: !canGoForward }} disabled={!canGoForward} onPress={onNext} style={[styles.navigationButton, { opacity: canGoForward ? 1 : 0.4 }]}><Text style={[styles.navigationButtonLabel, { color: theme.colors.primary }]}>›</Text></Pressable>
  </View>;
}
